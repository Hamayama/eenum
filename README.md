...
- eenum 手続きの書式は以下の通りです。  
  ...
  - キーワード引数 :en の exponential-notation においては、  
    指数表記の指定かつ整数部の桁数の指定を行います。  
    `#t` を指定すると、整数部を1桁として、指数表記による出力を行います。  
    また、数値を指定すると、整数部を指定した桁数として、指数表記による出力を行います。  
    `#f` を指定すると、指数表記による出力は行いません。  
    このキーワード引数を指定しなかった場合には、#f を指定したことになります。
...
2. SRFI-48 の format について  
   ...
   ( → 既知のバグについては、プルリクエストを送信しました。(2017-10-26)  
   https://github.com/scheme-requests-for-implementation/srfi-48/pull/1 )
