...
2. SRFI-48 の format について  
   ...
   また、参照実装のソースには、以下のバグがあるようです。(2017-10-24 確認)  
   ```
   (format "~1,2F" 5.015)    ==> "5.2."    (小数点が2個になっている)
   (format "~1,1F" -123.95)  ==> "-122.0"  (絶対値をとっていない?)
   (format "~1,2F" -999.995) ==> "-998.00" (絶対値をとっていない?)
   (format "~1,12F" 1/7)     ==> "0.1.4285714285" (小数点が2個になっている)
   (format "~1f" 1)          ==> error (幅を指定すると小文字のfを受け付けない)
   ```
   主にソースの「must round to shrink it」のあたりがおかしいようですが。。。
