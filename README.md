...
- eenum 手続きの書式は以下の通りです。  
  `eenum num [width] [digits] [round-mode] [pad-char]
   [plus-sign] [sign-align-left] [circular-digits]`
  ...
  - 第8引数の circular-digits には、循環小数の最大桁数を指定します。  
    num に有理数 (1/7 等) を指定すると 循環小数に展開しますが、  
    ここで指定した桁数までで展開を止めます。  
    第8引数に #f を指定した場合には、100 を指定したことになります。  
    第8引数は省略可能です。省略時は #f (すなわち 100) を指定したことになります。
