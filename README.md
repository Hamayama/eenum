...
- eenum 手続きの引数  
  ...
  `eenum num [width] [digits] [plus-sign] [pad-char] [round-mode]`
  ...
  - 第3引数の digits には、小数点以下の桁数を指定します。  
    ...
    また、第3引数に #f を指定した場合には、結果をそのまま出力します。  
  ...
  - 第6引数の round-mode には、丸めモードを指定します。  
    以下のいずれかを指定できます。
    ```
    ...
    'round2    四捨五入
    ```
    第6引数に #f を指定した場合には、'truncate を指定したことになります。  
    第6引数は省略可能です。省略時は #f (すなわち 'truncate) を指定したことになります。
